(function YouTubeSilenceSkipperPopup() {
  ...
  const { DEFAULT_SETTINGS, MESSAGE_TYPES, YOUTUBE_URL_PATTERN, LOG_LEVELS } = SHARED_CONSTANTS;
  ...
  const SettingsManager = {
    ...
    loadSettings: function(callback) {
      browser.storage.sync.get(DEFAULT_SETTINGS).then((settings) => {
        ...
        if (callback) callback(this.storedSettings);
      });
    },
    ...
    broadcastSettings: function(settings) {
      browser.tabs.query({ url: "https://www.youtube.com/*" }).then((tabs) => {
        for (const tab of tabs) {
          browser.tabs.sendMessage(tab.id, { type: MESSAGE_TYPES.SETTINGS_UPDATE, settings: settings });
        }
      });
    }
  };
