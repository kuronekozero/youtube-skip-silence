(function YouTubeSilenceSkipper() {
  ...
  const State = {
    ...
    loadSettings: function() {
      browser.storage.sync.get(DEFAULT_SETTINGS).then((res) => {
  ...
  const VideoChangeDetector = {
    ...
    checkVideoChange: function() {
      ...
      if (currentId && currentId !== this.lastVideoId) {
        ...
        browser.runtime.sendMessage({ type: MESSAGE_TYPES.RESET_FETCH });
  ...
  function requestCaptions() {
    ...
    if (State.isAdPlaying) {
      ...
      browser.runtime.sendMessage({ type: MESSAGE_TYPES.REQUEST_TIMEDTEXT });
    }
  }
